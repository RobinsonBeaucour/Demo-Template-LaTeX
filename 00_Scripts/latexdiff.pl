...
#  - Commands that were both in the safecmd and text command list, and which have additional arguments beyond the last textual one (optional or non-optional) could lead to errors, if there was a change in those earlier arguments  (fixes #306, first reported through debian bug report)
#
# Version 1.3.4:
# New features:
